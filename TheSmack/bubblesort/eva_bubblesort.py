...
def bubble_sort2(list):  # defines the function and passes in the parameter unsorted_list which is the list
    ...
    for e in range(length2 - 1):  # iterates through the length of the list - 1 since that is how many times is needed
        for f in range(length2 - 1 - e):  # the last element will already be sorted so iterates through length - 1 - e
            if len(list[f]) > len(list[f + 1]):  # if the element that comes first is bigger than the next...
                right2 = list[f]  # the variable "right" gets set to that number
                list[f] = list[f + 1]  # the bigger number takes the place of the right-most index
                list[f + 1] = right2  # it now assigns the number on the right to be the "right" variable
